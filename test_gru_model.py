...
if __name__ == '__main__':
    ...
    position_npz_path = sys.argv[3]
    velocity_npz_path = sys.argv[4]
    ...
    pos_input_mean, pos_input_std, pos_target_mean, pos_target_std = load_normalization_parameters(position_npz_path)
    vel_input_mean, vel_input_std, vel_target_mean, vel_target_std = load_normalization_parameters(velocity_npz_path)
    ...
    # Normalize the input sequence for the position model
    normalized_position_sequence = normalize_sequence(sequence[:-1], pos_input_mean, pos_input_std)
    # Predict the position trajectory using the normalized input
    t1_ns=time.time_ns()
    for i in range(100):
        predicted_normalized_position = predict_trajectory(position_model, normalized_position_sequence)
    t2_ns=time.time_ns()
    t_seconds = (t2_ns-t1_ns) * 1e-9
    print(f"Position prediction took average {t_seconds/100.} seconds\n")
    # Denormalize the predicted position trajectory
    predicted_position = denormalize_sequence(predicted_normalized_position, pos_target_mean, pos_target_std)


    # Predict the output velocity sequence using the input velocity
    # Normalize the input sequence for the velocity model
    normalized_velocity_input = normalize_sequence(input_velocity, vel_input_mean, vel_input_std)
    # Predict the output velocity sequence using the normalized input
    t1_ns = time.time_ns()
    for i in range(100):
        predicted_normalized_velocity_output = predict_trajectory(velocity_model, normalized_velocity_input)
    t2_ns = time.time_ns()
    t_seconds = (t2_ns-t1_ns) * 1e-9
    print(f"Velocity prediction took average {t_seconds/100.} seconds\n")
    # Denormalize the predicted velocity trajectory
    predicted_velocity_output = denormalize_sequence(predicted_normalized_velocity_output, vel_target_mean, vel_target_std)
    ...
    fig.savefig(traj_type+'_trajectory_mix-pos_mix-vel.pdf', format='pdf')
